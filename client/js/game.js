...
function(InfoManager, BubbleManager, Renderer, Map, Animation, Sprite, AnimatedTile,
    ...
    var Game = Class.extend({
        ...
        //
        // The connect() method creates a GameClient and wires it up to
        // the game.  Callback registrations are extracted into focused
        // sub-methods for readability:
        //
        //   connect()
        //     ├── onDispatched   → connect to actual game server
        //     ├── onConnected    → send HELLO handshake
        //     ├── _registerConnectionCallbacks()   (WELCOME, disconnect)
        //     ├── _registerWorldCallbacks()         (LIST, SPAWN)
        //     ├── _registerEntityCallbacks()        (despawn, destroy, move, teleport…)
        //     ├── _registerCombatCallbacks()         (attack, damage, kill)
        //     ├── _registerPlayerStateCallbacks()    (health, max HP)
        //     └── _registerUICallbacks()             (chat, population)
        //
        // Callbacks are registered before the connection is fully established.
        // This is safe — the on* methods only store function references;
        // no messages can arrive until the WebSocket handshake completes
        // and the server sends WELCOME in response to HELLO.
        // =================================================================
        ...
        connect: function(started_callback) {
            ...
            // Grouped by responsibility — see sub-methods below.
            this._registerConnectionCallbacks(started_callback);
            this._registerWorldCallbacks();
            this._registerEntityCallbacks();
            this._registerCombatCallbacks();
            this._registerPlayerStateCallbacks();
            this._registerUICallbacks();
        },

        // -----------------------------------------------------------------
        // CONNECTION callbacks — handshake completion and session teardown
        // -----------------------------------------------------------------

        _registerConnectionCallbacks: function(started_callback) {
            var self = this;

            // WELCOME — first game message, establishes player identity
            this.client.onWelcome(function(id, name, x, y, hp) {
                ...
                // These must be registered after welcome because they reference
                // self.playerId which is set above.
            ...
            // Disconnect — session lost or timeout
        ...
        // -----------------------------------------------------------------
        // WORLD callbacks — entity list sync and spawn handling
        // -----------------------------------------------------------------

        _registerWorldCallbacks: function() {
            var self = this;

            // Entity list — server tells us which entities exist in our zone
            ...
            // Spawn item
            this.client.onSpawnItem(function(item, x, y) {
                log.info("Spawned " + Types.getKindAsString(item.kind) + " (" + item.id + ") at "+x+", "+y);
                self.addItem(item, x, y);
            });

            // Spawn chest
            this.client.onSpawnChest(function(chest, x, y) {
            ...
            // Spawn character (player, mob, NPC)
            this.client.onSpawnCharacter(function(entity, x, y, orientation, targetId) {
        ...
        // -----------------------------------------------------------------
        // ENTITY callbacks — despawn, destroy, movement, teleport, equip
        // -----------------------------------------------------------------

        _registerEntityCallbacks: function() {
            var self = this;

            // Despawn entity
            this.client.onDespawnEntity(function(entityId) {
            ...
            // Entity destroyed (immediate removal)
            ...
            // Player moving toward item (other players)
            ...
            // Item blink effect
            this.client.onItemBlink(function(id) {
            ...
            // Player equipped item (visual update)
        ...
        // -----------------------------------------------------------------
        // COMBAT callbacks — attack links, damage info, kill tracking
        // -----------------------------------------------------------------

        _registerCombatCallbacks: function() {
            var self = this;
            ...
            // Player damaged a mob (show damage number)
        ...
        // -----------------------------------------------------------------
        // PLAYER STATE callbacks — health and hit points
        // -----------------------------------------------------------------

        _registerPlayerStateCallbacks: function() {
            var self = this;

            // Player health changed (damage or heal)
            ...
            // Player max hit points changed (level up, equipment)
        ...
        // -----------------------------------------------------------------
        // UI callbacks — chat and population display
        // -----------------------------------------------------------------

        _registerUICallbacks: function() {
            var self = this;
